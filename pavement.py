...
try:
    from wcmc_geonode.settings import GEONODE_APPS
except:
...
def start_geoserver(options):
    ...
    from wcmc_geonode.settings import OGC_SERVER
...
def _reset():
    sh("rm -rf wcmc_geonode/development.db")
    sh("rm -rf wcmc_geonode/uploaded/*")
    _install_data_dir()
